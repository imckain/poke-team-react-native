    ...
    if (el === true) {
      return (
        <Pressable onPressIn={() => setCollapsed(false)}>
          <Text style={[styles.baseStatHeaderText, { fontSize: headerFontSize }]}>Base Stats  <Entypo name="triangle-right" size={28} color="rgb(175, 175, 175)" /></Text>
        </Pressable>
      )
    } if (el === false) {
      return(
        <Pressable onPressIn={() => setCollapsed(true)}>
          <Text style={[styles.baseStatHeaderText, { fontSize: headerFontSize }]}>Base Stats  <Entypo name="triangle-down" size={28} color="rgb(175, 175, 175)" /></Text>
        </Pressable>
      )
    }
  }, []);
