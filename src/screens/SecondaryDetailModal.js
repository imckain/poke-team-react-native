...
const SecondaryDetailModal = (props) => {
  ...
  const isLocationAvailable = () => {
    if (urlResults[0] === undefined) return <View style={[styles.headerWrapper, { width: '90%' }]}><Text adjustsFontSizeToFit={true} numberOfLines={1} style={[styles.locationNavigation, {fontStyle: 'italic', color: 'rgba(255, 255, 255, 0.5)'}]}>Encounter Details Not Available</Text></View>
    else return (
      <View style={[styles.headerWrapper, {flexDirection: 'row', justifyContent: 'space-between', width: '100%', alignItems: 'center'}]}>
        ...
        <Entypo name="triangle-right" size={32} color="rgba(255, 255, 255, 0.5)" />
      </View>
    )
  }
